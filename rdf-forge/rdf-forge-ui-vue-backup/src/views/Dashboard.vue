    ...
    <div class="grid mt-4">
      <div class="col-12 lg:col-8">
        <Card>
          ...
          <template #content>
            <DataTable :value="recentJobs" :rows="5" responsiveLayout="scroll" :loading="loading">
              <Column field="pipeline" header="Pipeline">
                <template #body="{ data }">
                  <Skeleton v-if="loading" width="100%"></Skeleton>
                  <span v-else>{{ data.pipeline }}</span>
                </template>
              </Column>
              <Column field="status" header="Status">
                <template #body="{ data }">
                  <Skeleton v-if="loading" width="80px"></Skeleton>
                  <Tag v-else :severity="getStatusSeverity(data.status)">{{ data.status }}</Tag>
                </template>
              </Column>
              <Column field="startedAt" header="Started">
                <template #body="{ data }">
                  <Skeleton v-if="loading" width="100px"></Skeleton>
                  <span v-else>{{ formatDate(data.startedAt) }}</span>
                </template>
              </Column>
              <Column field="duration" header="Duration">
                <template #body="{ data }">
                  <Skeleton v-if="loading" width="60px"></Skeleton>
                  <span v-else>{{ data.duration }}</span>
                </template>
              </Column>
            </DataTable>
          </template>
