  ...
  <div class="pipeline-designer">
    ...
    <div class="designer-toolbar">
      <div class="toolbar-left">
        <InputText v-model="pipelineName" placeholder="Pipeline Name" class="pipeline-name-input" :class="{'p-invalid': !pipelineName}" />
        <Dropdown v-model="selectedFormat" :options="formats" placeholder="Format" class="w-8rem" />
      </div>
      <div class="toolbar-right">
        <Button label="Validate" icon="pi pi-check" class="p-button-outlined" @click="validatePipeline" :disabled="loading" />
        <Button label="Preview" icon="pi pi-eye" class="p-button-outlined" @click="previewPipeline" :disabled="loading" />
        <Button label="Save" icon="pi pi-save" class="p-button-primary" @click="savePipeline" :disabled="loading" />
        <Button label="Run" icon="pi pi-play" class="p-button-success" @click="runPipeline" :disabled="loading" />
      </div>
    </div>
...
const savePipeline = async () => {
  if (!pipelineName.value.trim()) {
    toast.add({ severity: 'error', summary: 'Validation Error', detail: 'Pipeline name is required', life: 3000 })
    return
  }
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to save pipeline', life: 3000 })
  } finally {
    loading.value = false
  }
}
...
const runPipeline = async () => {
  ...
  try {
    ...
    toast.add({ severity: 'success', summary: 'Started', detail: 'Pipeline execution started', life: 3000 })
  } catch (error) {
    toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to start pipeline', life: 3000 })
  } finally {
    loading.value = false
  }
}
