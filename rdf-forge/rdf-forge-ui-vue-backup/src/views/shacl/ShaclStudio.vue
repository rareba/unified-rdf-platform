...
const saveShape = async () => {
  if (!currentShape.value.name) {
    toast.add({ severity: 'error', summary: 'Validation Error', detail: 'Shape name is required', life: 3000 })
    return
  }
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to save shape', life: 3000 })
  } finally {
    loading.value = false
  }
}
...
const runValidation = async () => {
  ...
  try {
    ...
    toast.add({ severity: 'info', summary: 'Completed', detail: 'Validation completed', life: 3000 })
  } finally {
    loading.value = false
  }
}
