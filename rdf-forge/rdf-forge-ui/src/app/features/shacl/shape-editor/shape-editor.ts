...
export class ShapeEditor implements OnInit {
  ...
  loadShape(id: string): void {
    ...
    this.shaclService.get(id).subscribe({
      next: (shape) => {
        ...
        // Try to parse SHACL content to populate visual properties
        if (shape.content) {
          const parsedProps = this.parseShaclToProperties(shape.content);
          if (parsedProps.length > 0) {
            this.properties.set(parsedProps);
            // Stay in visual mode since we successfully parsed properties
            this.visualMode.set(true);
          } else {
            // Could not parse properties, but still start in visual mode
            // User can switch to code mode manually
            this.visualMode.set(true);
          }
        }
  ...
  // Parse SHACL Turtle content to extract property shapes for visual editor
  private parseShaclToProperties(content: string): PropertyShape[] {
    const properties: PropertyShape[] = [];

    try {
      // Simple regex-based parser for common SHACL property patterns
      // This handles the most common cases generated by our visual editor
      const propertyBlockRegex = /sh:property\s*\[([\s\S]*?)\]\s*[;.]/g;
      let match;

      while ((match = propertyBlockRegex.exec(content)) !== null) {
        const block = match[1];

        const prop: PropertyShape = {
          id: crypto.randomUUID(),
          path: this.extractValue(block, /sh:path\s+<([^>]+)>/),
          name: this.extractQuotedValue(block, /sh:name\s+"([^"]+)"/),
          description: this.extractQuotedValue(block, /sh:description\s+"([^"]+)"/) || undefined,
          minCount: this.extractNumber(block, /sh:minCount\s+(\d+)/),
          maxCount: this.extractNumber(block, /sh:maxCount\s+(\d+)/),
          datatype: this.extractValue(block, /sh:datatype\s+(\S+)/) || 'xsd:string',
          kind: block.includes('sh:nodeKind sh:IRI') || block.includes('sh:class') ? 'RESOURCE' : 'LITERAL',
          pattern: this.extractQuotedValue(block, /sh:pattern\s+"([^"]+)"/) || undefined,
          minLength: this.extractNumber(block, /sh:minLength\s+(\d+)/) || undefined,
          maxLength: this.extractNumber(block, /sh:maxLength\s+(\d+)/) || undefined,
          message: this.extractQuotedValue(block, /sh:message\s+"([^"]+)"/) || undefined,
          nodeKind: this.extractValue(block, /sh:nodeKind\s+(sh:\w+)/) || undefined
        };

        // Only add if we found at least a path
        if (prop.path) {
  ...
  private extractQuotedValue(text: string, regex: RegExp): string {
    const match = text.match(regex);
    return match ? match[1] : '';
  }
