...
public interface ShapeRepository extends JpaRepository<ShapeEntity, UUID> {
    ...
    @Query("SELECT s FROM ShapeEntity s WHERE s.projectId = :projectId AND " +
           "(LOWER(s.name) LIKE LOWER(CONCAT('%', :search, '%')) OR " +
           "LOWER(s.description) LIKE LOWER(CONCAT('%', :search, '%')) OR " +
           "LOWER(s.targetClass) LIKE LOWER(CONCAT('%', :search, '%')))")
    Page<ShapeEntity> searchByProjectId(@Param("projectId") UUID projectId, 
                                         @Param("search") String search, 
                                         Pageable pageable);
    
    @Query("SELECT DISTINCT s.category FROM ShapeEntity s WHERE s.projectId = :projectId AND s.category IS NOT NULL")
    List<String> findCategoriesByProjectId(@Param("projectId") UUID projectId);
}
